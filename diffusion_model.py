...
def parse_args():
    ...
    parser.add_argument("--ddim_num_inference_steps", type=int, default=1000)
...
def main(args):
    #initialize the vae
    vae = vae = AutoencoderKL.from_pretrained("CompVis/stable-diffusion-v1-4", subfolder="vae",torch_dtype=torch.float16).to("cuda")
    ...
    for epoch in range(first_epoch, args.num_epochs):
        ...
        for step, batch in enumerate(train_dataloader):
            ...
            latent_image = vae.encode(clean_images.to(dtype=torch.float16).to("cuda")).latent_dist.sample()
            latent_image = latent_image*0.1825 
            ...
            timesteps = torch.randint(
                0, noise_scheduler.config.num_train_timesteps, (bsz,), device=latent_image.device
            ).long()

            # Add noise to the clean images according to the noise magnitude at each timestep
            # (this is the forward diffusion process)
            noisy_images = noise_scheduler.add_noise(latent_image, noise, timesteps)
            ...
            with accelerator.accumulate(model):
                ...
                if args.prediction_type == "epsilon":
                    ...
                    alpha_t = _extract_into_tensor(
                        noise_scheduler.alphas_cumprod, timesteps, (latent_image.shape[0], 1, 1, 1)
                    )
                    snr_weights = alpha_t / (1 - alpha_t)
                    loss = snr_weights * F.mse_loss(
                        model_output, latent_image, reduction="none"
